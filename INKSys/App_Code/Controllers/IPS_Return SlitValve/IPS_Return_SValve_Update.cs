...
public class IPS_Return_SValve_Update
{
    string sqlconn = ConfigurationManager.AppSettings["dbINKSYS"];

    public string updateIPSReturnSValve(string returnlotno, string partlotno, string amount, string cavityno, string boxno)
    {
        string Message = "";
        try
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("USP_IPS_ReturnSValve_Patch", conn);
